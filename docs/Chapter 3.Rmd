...
\begin{align}
  \begin{aligned}\nonumber
    ...
    \sum_{i=1}^m \sum_{j=1}^n \sum_{t=1}^T u_{i,j}(t) \text{log } p_{i,j}(x_t)  & \qquad \lambda_{b;i}        &\hat{\lambda}_{b;i} = \frac{\sum_{j=1}^n \sum_{t=1}^T \hat{u}_j(t) b_t}{\sum_{t=1}^T \hat{u}_j(t)} \\
                                                                                & \qquad \lambda_{s;j}        &\hat{\lambda}_{s;j} = \frac{\sum_{i=1}^m \sum_{t=1}^T \hat{u}_i(t) s_t}{\sum_{t=1}^T \hat{u}_i(t)}
  \end{aligned}
\end{align}
