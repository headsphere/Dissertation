...
We repeat the above iterative scheme a large number of times until we meet our convergence criteria. In this instance we repeat until the marginal improvement in the CDDL is less than $10^{-6}.$\marginnote{The R code for this algorithm can be found in appendix TODO}

The problem remains that without suitable starting values for the initial distribution $\boldsymbol{\delta}$, and the transition matrix $\boldsymbol{\Gamma}$, the EM algorithm maay converge on local not global optima. Therefore a strategy for mitigating this problem is required.
