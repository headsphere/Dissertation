...
```{r, fig.width = 10, fig.height = 2, fig.fullwidth = TRUE, echo=FALSE, fig.cap="Kernel density estimate of simulated Poisson mixture (bandwidth=30, ker- nel=gaussian)"}
ggplot(df_melt,aes(x=x, fill=variable)) + geom_density(alpha=0.25)
```

The kernel density estimates are generated with the Gaussian kernel and a bandwidth of `r round(bw.nrd0(x),2)`. The multimodal appearance eof the plot clearly shows that the data are overdispersed compared to a single Poisson, and instead appear to come from a mixture of two Poissons. 
