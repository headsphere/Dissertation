...
The first step to calculate the likelihood fuinction is to extend the univariate marginal distribution $\eqref{eq:marginalNonStat}$ to a bivariate distribution. Zucchini and MacDonald show that $\eqref{eq:marginalNonStat}$ can be extended so that:
...
This then can be generalised to a $T$-th order distribution and hence the likelihood for a set of observations $\mathbf{X}^{(T)} = (X_1,X,2,\dots,X_T)$ given the parameters of an HMM is:

$$L_T = \boldsymbol{\delta} \mathbf{P}(x_1) \boldsymbol{\Gamma} \mathbf{P}(x_2) \dots \boldsymbol{\Gamma} \mathbf{P}(x_T) \mathbf{1'}$$



- Marginal Multivariate HMM
