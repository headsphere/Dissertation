...
We can see from both the following plots of the time series and the histograms the data appears to conform to two separate distributions, or states, as it were:  

```{r echo=FALSE, fig.fullwidth = FALSE, fig.cap="Time series of simulated Poisson mixture"}
  plot.ts(x, xlab = NULL, ylab = NULL)
```
```{r echo=FALSE, fig.fullwidth = FALSE, fig.cap="Histogram of simulated Poisson mixture with fitted density"}
...
If we define $\{C_t : t = 1,2, \dots T \}$ as the unobserved hidden states, and $\{X_t : t = 1,2, \dots T \}$ as the observed data points, we can say a hidden Markov is a dependent mixture where:
...
The $m \times m$ diagonal matrix of conditional probabilities ${P}(x)$ defined as:
...
After the E step is complete, the M Step is now to maximise the CDDL \eqref{eq:cddl} with respect to the three sets of parameters of interest, $\boldsymbol{\delta}$, $\boldsymbol{\Gamma}$ and the parameters of the state-dependent distributions, namely $\lambda_1$,...,$\lambda_m$.^[We replace $p_j(x_t)$ in the CDDL with $e^{-\lambda_j} \lambda_j^x / x!$]
...
\begin{align}
  \begin{aligned}\nonumber
    ...
    \sum_{j=1}^m \sum_{k=1}^m \Bigg ( \sum_{t=2}^T \hat{v}_{jk}(t)   \Bigg ) \text{log } \gamma_{jk} & \qquad \boldsymbol{\Gamma} & \gamma_{jk} = \frac{f_{jk}}{\sum_{k=1}^m f_{jk}} \\
    & & \text{ with } f_{jk} = \sum_{t=2}^T \hat{v}_{jk}(t)\\
